Vue.component('stm-search-form-category', {
    ...
    methods: {
                        ...
                        if (result.hasOwnProperty('success') && result.success) {
                            ...
                            if (vm.hasAccess && vm.result && vm.result.length > 0) {
                                vm.result.forEach(function (elem, index) {
                                    ...
                                    elem.post_title = vm.markTitle(elem.post_title, vm.currentElem.target.value, 'span');
        ...
        markTitle: function (text, words, tag) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (re.test(text)) {
                    text = text.replace(re, '<'+ tag +' class="uListing-title-text-shadow">$&</'+ tag +'>');
                }
            }
            return text;
        },
