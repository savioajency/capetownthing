...
jQuery( document ).ready( function() {
	...
	function lione_init_actions() {
		...
		if ( $body.hasClass( 'menu_side_present' ) ) {
			if ( $menu_side_logo.length > 0 && ! lione_is_images_loaded( $menu_side_logo ) ) {
	...
	function lione_ready_actions() {
		...
		//------------------------------------
		jQuery( 'a.lione_skip_link:not(.inited)' )
			.addClass( 'inited' )
			.on( 'focus', function() {
		...
		if ( jQuery.ui && jQuery.ui.tabs ) {
			jQuery( '.lione_tabs:not(.inited)' ).each(
				function () {
					var $self = jQuery( this );
		...
		if (jQuery.ui && jQuery.ui.accordion) {
			jQuery( '.lione_accordion:not(.inited)' )
				.addClass( 'inited' )
				.accordion( {
		...
		$document.on(
			'action.scroll_lione', function(e) {
		...
		LIONE_STORAGE['cur_page_url']   = location.href;
		LIONE_STORAGE['cur_page_title'] = jQuery('.sc_layouts_title_caption').eq(0).text() || jQuery('head title').eq(0).text() || '';
		$document.on(
			'action.scroll_lione', function(e) {
				...
				var container      = LIONE_STORAGE['which_block_load'] == 'article'
					...
					cur_page_link  = LIONE_STORAGE['cur_page_url'],
					cur_page_title = LIONE_STORAGE['cur_page_title'];
	...
	function lione_init_media_elements(cont) {
		var audio_selector = lione_apply_filters( 'lione_filter_mediaelements_audio_selector', 'audio:not(.inited)' ),
	...
	function lione_tabs_ajax_content_loader(panel, page, oldPanel) {
		...
		jQuery.post(
			...
			if (rez.error !== '') {
				...
				// Append posts
				panel
					.find( '.lione_tab_holder,.lione_tab_content_remove' ).remove().end()
	...
	// Get inline styles and add to the page styles
	function lione_import_inline_styles( response ) {
		var selector = 'lione-inline-styles-inline-css';
	...
	// Get tags 'link' from response and add its to the 'head'
	function lione_import_tags_link( response ) {
	...
	function lione_resize_video(cont) {
		...
		if ( LIONE_STORAGE['resize_tag_iframe'] ) {
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				var $self = jQuery( this );
				// If item now invisible
				if ( $self.hasClass( 'trx_addons_resize' ) || $self.hasClass( 'trx_addons_noresize' ) || $self.addClass( 'lione_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0 ) {
					return;
				}
