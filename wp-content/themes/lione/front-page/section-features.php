<div class="front_page_section front_page_section_features<?php
	$lione_scheme = lione_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $lione_scheme ) && ! lione_is_inherit( $lione_scheme ) ) {
		echo ' scheme_' . esc_attr( $lione_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( lione_get_theme_option( 'front_page_features_paddings' ) );
	if ( lione_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$lione_css      = '';
		$lione_bg_image = lione_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$lione_anchor_icon = lione_get_theme_option( 'front_page_features_anchor_icon' );
	$lione_anchor_text = lione_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( lione_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$lione_css      = '';
			$lione_bg_mask  = lione_get_theme_option( 'front_page_features_bg_mask' );
			$lione_bg_color_type = lione_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $lione_bg_color_type ) {
				$lione_bg_color = lione_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$lione_caption = lione_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $lione_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $lione_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $lione_caption, 'lione_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$lione_description = lione_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $lione_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $lione_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $lione_description ), 'lione_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! lione_exists_trx_addons() ) {
						...
						lione_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
