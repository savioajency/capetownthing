...
if ( ! function_exists( 'lione_storage_get' ) ) {
	/**
	 ...
	 */
	function lione_storage_get( $var_name, $default = '' ) {
		global $LIONE_STORAGE;
		return isset( $LIONE_STORAGE[ $var_name ] ) ? $LIONE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'lione_storage_set' ) ) {
	/**
	 ...
	 */
	function lione_storage_set( $var_name, $value ) {
		global $LIONE_STORAGE;
		$LIONE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'lione_storage_empty' ) ) {
	/**
	 ...
	 */
	function lione_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $LIONE_STORAGE;
		if ( ! empty( $key ) && ! empty( $key2 ) ) {
			return empty( $LIONE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( ! empty( $key ) ) {
			return empty( $LIONE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $LIONE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'lione_storage_isset' ) ) {
	/**
	 ...
	 */
	function lione_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $LIONE_STORAGE;
		if ( ! empty( $key ) && ! empty( $key2 ) ) {
			return isset( $LIONE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( ! empty( $key ) ) {
			return isset( $LIONE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $LIONE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'lione_storage_unset' ) ) {
	/**
	 ...
	 */
	function lione_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $LIONE_STORAGE;
		if ( ! empty( $key ) && ! empty( $key2 ) ) {
			unset( $LIONE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( ! empty( $key ) ) {
			unset( $LIONE_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $LIONE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'lione_storage_inc' ) ) {
	/**
	 ...
	 */
	function lione_storage_inc( $var_name, $value = 1 ) {
		global $LIONE_STORAGE;
		if ( empty( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = 0;
		}
		$LIONE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'lione_storage_concat' ) ) {
	/**
	 ...
	 */
	function lione_storage_concat( $var_name, $value ) {
		global $LIONE_STORAGE;
		if ( empty( $LIONE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'lione_storage_get_array' ) ) {
	/**
	 ...
	 */
	function lione_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $LIONE_STORAGE;
		if ( empty( $key2 ) ) {
			return ! empty( $var_name ) && ! empty( $key ) && isset( $LIONE_STORAGE[ $var_name ][ $key ] ) ? $LIONE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && ! empty( $key ) && isset( $LIONE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $LIONE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'lione_storage_set_array' ) ) {
	/**
	 ...
	 */
	function lione_storage_set_array( $var_name, $key, $value ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$LIONE_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'lione_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function lione_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $LIONE_STORAGE[ $var_name ][ $key ] ) ) {
			$LIONE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$LIONE_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$LIONE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'lione_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function lione_storage_merge_array( $var_name, $key, $value ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$LIONE_STORAGE[ $var_name ] = array_merge( $LIONE_STORAGE[ $var_name ], $value );
		} else {
			$LIONE_STORAGE[ $var_name ][ $key ] = array_merge( $LIONE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'lione_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function lione_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			lione_array_insert_after( $LIONE_STORAGE[ $var_name ], $after, $key );
		} else {
			lione_array_insert_after( $LIONE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'lione_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function lione_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			lione_array_insert_before( $LIONE_STORAGE[ $var_name ], $before, $key );
		} else {
			lione_array_insert_before( $LIONE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'lione_storage_push_array' ) ) {
	/**
	 ...
	 */
	function lione_storage_push_array( $var_name, $key, $value ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $LIONE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $LIONE_STORAGE[ $var_name ][ $key ] ) ) {
				$LIONE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $LIONE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'lione_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function lione_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $LIONE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $LIONE_STORAGE[ $var_name ] ) && is_array( $LIONE_STORAGE[ $var_name ] ) && count( $LIONE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $LIONE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $LIONE_STORAGE[ $var_name ][ $key ] ) && is_array( $LIONE_STORAGE[ $var_name ][ $key ] ) && count( $LIONE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $LIONE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'lione_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function lione_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $LIONE_STORAGE[ $var_name ][ $key ] ) ) {
			$LIONE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$LIONE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'lione_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function lione_storage_concat_array( $var_name, $key, $value ) {
		global $LIONE_STORAGE;
		if ( ! isset( $LIONE_STORAGE[ $var_name ] ) ) {
			$LIONE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $LIONE_STORAGE[ $var_name ][ $key ] ) ) {
			$LIONE_STORAGE[ $var_name ][ $key ] = '';
		}
		$LIONE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'lione_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function lione_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $LIONE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $LIONE_STORAGE[ $var_name ] ) ? $LIONE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $LIONE_STORAGE[ $var_name ] ) ? $LIONE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'lione_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function lione_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $LIONE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $LIONE_STORAGE[ $var_name ]->$prop ) ? $LIONE_STORAGE[ $var_name ]->$prop : $default;
	}
}
