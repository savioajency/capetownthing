...
jQuery( document ).ready(
	function() {
		...
		lione_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="lione_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.lione_post_editor' );
						if ( link.length > 0 ) {
							lione_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.lione_post_editor', function(e) {
						lione_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('lione_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function lione_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
