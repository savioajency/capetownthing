...
if ( function_exists( 'lione_elementor_is_preview' ) && lione_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			lione_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	lione_new_main_query(
		array(
			'post_type'      => lione_get_theme_option( 'post_type' ),
			'category'       => lione_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'lione_filter_get_template_part', lione_blog_archive_get_template() ) );
}
