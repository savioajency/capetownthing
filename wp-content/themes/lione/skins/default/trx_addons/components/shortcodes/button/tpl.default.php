	...
	foreach ($args['buttons'] as $item) {
		...
		?><a href="<?php echo esc_url($item['link']); ?>"<?php
			...
			if (!empty($item['image']) || !empty($item['icon'])) {
				?><span class="sc_button_icon"><?php
					if (!empty($item['image'])) {
						...
						?><img class="sc_icon_as_image" src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'lione'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						...
						if (strpos($item['icon'], '.svg') !== false) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($item['icon']); ?>" alt="<?php esc_attr_e('Icon', 'lione'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						}
					} else {
            ...
            if ( $item['type'] === 'animated' ) :
	            $item_id = trx_addons_generate_id( 'sc_button_canvas_' );
	            ?>
                <span class="sc_button_canvas_circle">
                    <canvas id="<?php echo esc_attr( $item_id ); ?>"></canvas>
                </span>
            <?php endif;
