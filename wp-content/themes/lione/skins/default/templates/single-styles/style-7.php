...
if ( apply_filters( 'lione_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$lione_with_featured_image = lione_is_with_featured_image( $lione_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $lione_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $lione_with_featured_image ) {
                echo ' with_featured_image' . ( false && lione_get_theme_option( 'single_parallax' ) == 0 ? ' lione-full-height' : '' );
            }
		?>">
			<?php
