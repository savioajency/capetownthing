...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $lione_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	lione_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'lione_filter_related_thumb_size', lione_get_thumb_size( (int) lione_get_theme_option( 'related_posts' ) == 1 ? 'big' : 'med' ) ),
		)
	);
	?>
	<div class="post_header entry-header">
		<h6 class="post_title entry-title"><a href="<?php echo esc_url( $lione_link ); ?>"><?php
			if ( '' == get_the_title() ) {
				esc_html_e( '- No title -', 'lione' );
			} else {
				the_title();
			}
		?></a></h6>
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<div class="post_meta">
				<a href="<?php echo esc_url( $lione_link ); ?>" class="post_meta_item post_date"><?php echo wp_kses_data( lione_get_date() ); ?></a>
			</div>
			<?php
		}
		?>
